...
class BYDFiRequestData(Feed):
    """BYDFi REST API Feed base class."""
    ...
    def __init__(self, data_queue: Any = None, **kwargs: Any) -> None:
        ...
        self._params = BYDFiExchangeDataSpot()
        ...
        self.request_logger = get_logger("bydfi_feed")
        self.async_logger = get_logger("bydfi_feed")
        self._http_client = HttpClient(venue=self.exchange_name, timeout=10)

    def _generate_signature(self, timestamp, params=None, body=""):
        """Generate HMAC SHA256 signature for BYDFi API.

        BYDFi signature format:
    ...
    async def async_request(self, path, params=None, body=None, extra_data=None, timeout=5):
        """Async HTTP request for BYDFi API."""
        method = path.split()[0] if " " in path else "GET"
        request_path = path.split()[1] if " " in path else path
